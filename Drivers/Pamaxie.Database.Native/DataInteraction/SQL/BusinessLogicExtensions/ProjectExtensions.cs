...
public static class ProjectExtensions
{
    public static IPamProject ToUserLogic(this Project project)
    {
        var pamProject = new PamProject()
        {
            ...
            LastModifiedUser = new LazyObject<(PamUser User, long UserId)>(),
            Owner = new LazyObject<(PamUser User, long UserId)>() {Data = (null, UserId: project.OwnerId), IsLoaded = false},
            ApiTokens = new LazyList<(string Token, long TokenId)>(){IsLoaded = false},
